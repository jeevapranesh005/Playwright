...
- Name: DataDriven.test.ts >> test of valid login
...
Call log:
  ...
  - waiting for locator('#nameofuser')
    16 × locator resolved to <a href="#" id="nameofuser" class="nav-link"></a>
...
```ts
  ...
  36 |         
  37 |         
  38 |         if(i.expectedresult==='succes'){
  39 |             await page.locator("//button[text()='Log in']").click();
  40 | 
  41 |             let res =  page.locator("#nameofuser")
> 42 |             await expect(res).toBeVisible({timeout:6000})
     |                               ^ Error: expect(locator).toBeVisible() failed
  43 |             console.log(await res.textContent())
  44 | 
  45 |         }
  46 | 
  47 |     })
